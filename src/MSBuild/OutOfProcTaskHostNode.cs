...
namespace Microsoft.Build.CommandLine
{
        ...
        INodePacketFactory, INodePacketHandler, IBuildEngine10
    {
        ...
        /// <summary>
        /// Number of tasks currently yielded (blocked on a BuildProjectFile callback).
        ...
        public bool BuildProjectFilesInParallel(string[] projectFileNames, string[] targetNames, IDictionary[] globalProperties, IDictionary[] targetOutputsPerProject, string[] toolsVersion, bool useResultsCache, bool unloadProjectsOnCompletion)
        {
            ...
            ErrorUtilities.VerifyThrow(
                ...
                "projectFileNames has {0} entries but targetOutputsPerProject has {1} ΓÇö lengths must match.",
            ...
            where TResponse : class, INodePacket
        {
            ...
            try
            {
                ...
                // Block until the response arrives (via HandleCallbackResponse ΓåÆ TCS.SetResult)
                // or the connection is lost (via OnLinkStatusChanged ΓåÆ TCS.TrySetException).
        ...
        /// Saves the current operating environment before yielding.
        /// </summary>
        ...
        /// <summary>
        /// Marks this TaskHost as active again after a callback completes.
        ...
        private void CompleteTask()
        {
            ...
            // that no task is executing here. The task that completed enqueued to _taskCompleteQueue
            // and signaled _taskCompleteEvent, but another task may have reacquired from yield
            // by the time this method runs.
            if (_nodeEndpoint.LinkStatus == LinkStatus.Active)
            {
                // Drain all queued completion packets ΓÇö multiple tasks may have completed
        ...
        {
            // Cancel all active task wrappers — with nested builds via yield-for-callback,
        ...
        private void RunTask(object state)
        {
            ...
            try
            {
                ...
                // when multiple tasks are active (nested builds via yield-for-callback).
                if (taskContext is not null)
                {
            ...
            finally
            {
                try
                {
                    ...
                    if (taskContext is not null && taskContext.State == TaskExecutionState.Yielded)
                    {
                        ...
                        // Don't decrement _activeTaskCount ΓÇö it was already decremented by YieldForCallback
                    }
                    else
                    {
                ...
                finally
                {
                    ...
                    // Use local variable ΓÇö _taskWrapper may have been overwritten by a nested task.
