...
for d in sys.argv[1:]:
    ...
    gpf = bf.get('cfg_globalProps_full',[0,0])
    gpi = bf.get('cfg_globalProps_identical',[0,0])
    if gpf[0] or gpi[0]:
        sends = gpf[0] + gpi[0]
        print(f"\n  globalProps wire mode: {gpf[0]} Full ({fmt_mb(gpf[1])}) + {gpi[0]} Identical ({fmt_mb(gpi[1])})  =>  "
              f"Full {100.0*gpf[0]/sends:.1f}% of {sends} sends; avg Full = {(gpf[1]/gpf[0]) if gpf[0] else 0:,.0f} B")
